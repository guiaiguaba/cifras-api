...
// Cloudinary — usado para deletar áudios de gravações de linha
...
// ---------------------------------------------------------------------
// Gravações — deleção no Cloudinary (credenciais ficam só no servidor)
// ---------------------------------------------------------------------

// DELETE /gravacao?publicId=xxx
// Chamado pelo app Flutter após deletar o doc no Firestore.
// Autentica via x-api-key igual às outras rotas.
app.delete('/gravacao', exigirApiKey, async (req, res) => {
  const { publicId } = req.query;
  if (!publicId) {
    return res.status(400).json({ erro: 'publicId é obrigatório.' });
  }
  try {
    const result = await cloudinary.uploader.destroy(publicId, {
      resource_type: 'raw',
    });
    // result.result pode ser 'ok' ou 'not found' — ambos são aceitáveis
